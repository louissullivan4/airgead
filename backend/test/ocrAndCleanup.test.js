...
const { cleanReceipt } = require('../src/utils/receiptCleanup');
...
describe('receiptCleanup.cleanReceipt', () => {
    it('returns a non-empty binarised buffer and cropped:false (crop deferred)', async () => {
        const input = await makeTestImage();
        const { binarisedBuffer, cropped } = await cleanReceipt(input);

        expect(Buffer.isBuffer(binarisedBuffer)).toBe(true);
        expect(binarisedBuffer.length).toBeGreaterThan(0);
        expect(cropped).toBe(false);

        // Output is a PNG and bilevel (≤2 colours) — the cheap stored format.
        const meta = await sharp(binarisedBuffer).metadata();
